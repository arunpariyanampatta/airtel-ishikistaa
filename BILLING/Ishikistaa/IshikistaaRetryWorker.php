...
EOD
        ...
        if (!$resulResp) {
            ...
            $conn = connect_db();
            $date = date("Ymd");
            $sql = "INSERT INTO `balance_assessment_logs_" . date("Ymd") . "` (`MSISDN`,`STATUS_CODE`,`BALANCE`) VALUES('" . $msisdn . "','1004','0')";
            ...
            return true;
             ...
             if (strpos($resulResp, 'Internal server error') != false) {
                 $nextRetrialPeriod = date("Y-m-d H:i:s",strtotime("+10 minutes"));
                 $conn = connect_db();
            $date = date("Ymd");
            $sql = "INSERT INTO `balance_assessment_logs_" . date("Ymd") . "` (`MSISDN`,`STATUS_CODE`,`BALANCE`) VALUES('" . $msisdn . "','1004','0')";
            ...
            if ($balancestatusCode == "0") {
                ...
                if ($balance < 10) {
                    ...
                    $conn = connect_db();
                    mysqli_query($conn, $sql);
                    $sql = "UPDATE tbl_subscribers SET   RETRY_STATUS = '0',QUEUE_STATUS = '1',NEXT_RETRIAL_PERIOD='".$nextRetrialPeriod."', RETRY_COUNT = CASE WHEN RETRY_COUNT IN (NULL,0,'') THEN 1 ELSE  RETRY_COUNT+1 END WHERE MSISDN = '" . $msisdn . "'";
                    mysqli_query($conn, $sql);
                    mysqli_close($conn);
                    return true;
                }
                ...
                if ($balancestatusCode == "102") {//not a prepaid customer
                    $sql = "INSERT INTO `balance_assessment_logs_" . date("Ymd") . "` (`MSISDN`,`STATUS_CODE`,`BALANCE`) VALUES('" . $msisdn . "','" . $balancestatusCode . "','0')";
                    $conn = connect_db();
                    mysqli_query($conn, $sql);
                ...
                if ($balancestatusCode == "126") {// if number is not active
                    ...
                    $phone = $msisdn;
                    $msg = urlencode($sms);
                    $sql = "update  tbl_subscribers SET QUEUE_STATUS = '0',RETRY_STATUS = '1'  where MSISDN = '" . $phone . "'";
                    $conn = connect_db();
                    mysqli_query($conn, $sql);
                    mysqli_close($conn);
                    return true;
                ...
                if ($balancestatusCode == "100") {//incase of any other error
                    ...
                    mysqli_query($conn, $sql);
                    $phone = $msisdn;
                    $sql = "UPDATE  tbl_subscribers SET QUEUE_STATUS = '1',RETRY_STATUS = '0',NEXT_RETRIAL_PERIOD = '".$nextRetrialPeriod."'   WHERE MSISDN = '" . $phone . "'";
                    $conn = connect_db();
                    mysqli_query($conn, $sql);
                    mysqli_close($conn);
                    return true;
...
EOD
    ...
    if (!$data) {
        ...
        if (strpos($data, 'Internal server error') != false) {
            $nextRetrialPeriod = date("Y-m-d H:i:s",strtotime("+10 minutes"));
            $date = date("Ymd");
        ...
        if (!isset($decoderesponse["responseCode"])) {
            ...
            if ($status == "0") {
                ...
                $nextRetrialPeriod = date("Y-m-d H:i:s",strtotime("+60 minutes"));
                $sql = "UPDATE tbl_subscribers SET  TOTAL_AMOUNT_CHARGED = " . $totAmount . ", NEXT_AMOUNT_CHARGE =  " . $nextChargeAmount . ",NEXT_RETRIAL_PERIOD = '".$nextRetrialPeriod."', RETRY_STATUS = '" . $retry . "',QUEUE_STATUS = '" . $queueStatus . "',RETRY_COUNT = CASE WHEN RETRY_COUNT IN (NULL,0,'') THEN 1 ELSE  RETRY_COUNT+1 END WHERE MSISDN = '" . $msisdn . "'";
                mysqli_query($conn, $sql);
                $date = date("Ymd");
                $sql = "INSERT INTO `vas_transaction_ishikistaa_" . $date . "`(`MSISDN`,`STATUS`,`STATUS_CODE`,`TXN_ID`,`TXN_AMOUNT`,`REQUEST_SENT`,`RESPONSE_RECEIVED`) VALUES ('" . $msisdn . "','" . $chargingStatus . "','" . $status . "','" . $transactionID . "'," . $amount . ",'" . $xml_data . "','" . $data . "')";
                mysqli_query($conn, $sql);
