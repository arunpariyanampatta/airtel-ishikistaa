        ...
        if (!$resulResp) {
            ...
            if ($balancestatusCode == "0") {
                ...
                if ($balance < 10) {
                    ...
                    $conn = connect_db();
                    mysqli_query($conn, $sql);
                    $sql = "UPDATE tbl_subscribers SET   RETRY_STATUS = '0',QUEUE_STATUS = '1',RETRY_COUNT = CASE WHEN RETRY_COUNT IN (NULL,0,'') THEN 1 ELSE  RETRY_COUNT+1 END WHERE MSISDN = '" . $msisdn . "'";
                    mysqli_query($conn, $sql);
                    mysqli_close($conn);
                    return true;
                }
                ...
                if ($balancestatusCode == "100") {//incase of any other error
                    ...
                    $sql = "update  tbl_subscribers SET QUEUE_STATUS = '1',RETRY_STATUS = '0', RETRY_COUNT = CASE WHEN RETRY_COUNT IN (NULL,0,'') THEN 1 ELSE  RETRY_COUNT+1 END  where MSISDN = '" . $phone . "'";
                    $conn = connect_db();
                    mysqli_query($conn, $sql);
                    mysqli_close($conn);
                    return true;
                    ...
                    $conn = connect_db();
                    mysqli_query($conn, $sql);
                    $sql = "update  tbl_subscribers SET QUEUE_STATUS = '1',RETRY_STATUS = '0', RETRY_COUNT = CASE WHEN RETRY_COUNT IN (NULL,0,'') THEN 1 ELSE  RETRY_COUNT+1 END  where MSISDN = '" . $msisdn . "'";
                    mysqli_query($conn, $sql);
                    mysqli_close($conn);
                    return true;
                }
