...
EOD
    ...
    if (!$resulResp) {// if there is no response return back to queue
        ...
        if (!xmlrpc_decode($resulResp)) {// f there is any malformed xml retrun bakc to queue
            return false;
        }
        ...
        if (!isset($decoderesponse["responseCode"])) {// if it is not showing any responsecode return back to queue
		    ...
		    $balance = 0;	
                ...
                if ($balancestatusCode == "100") {//incase of any other error
                    ...
                    $sql = "update  tbl_subscribers SET QUEUE_STATUS = '1',RETRY_STATUS = '0'  where MSISDN = '" . $phone . "'";
                    $conn = connect_db();
                    mysqli_query($conn, $sql);
                    mysqli_close($conn);
                    return true;
...
EOD
    ...
    if (!$data) {// if not able to fetch response update queue status to 1 and retry status 0
      $sql = "UPDATE tbl_subscribers SET  QUEUE_STATUS = '1', RETRY_STATUS = '0'  WHERE MSISDN = '" . $msisdn . "'";
